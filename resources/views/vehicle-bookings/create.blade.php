<x-app-layout>
    <x-slot name="header"><h1 class="text-xl font-semibold text-gray-800">จองยานพาหนะ</h1></x-slot>
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6"><a href="{{ route('vehicle-bookings.index') }}" class="hover:text-navy-600">รายการจอง</a><i data-lucide="chevron-right" class="w-4 h-4"></i><span class="text-gray-800">จองใหม่</span></div>
    ...
    <div class="card max-w-3xl">
        <div class="card-header"><h3 class="text-lg font-semibold text-gray-800">แบบฟอร์มจองยานพาหนะ</h3></div>
        <div class="card-body">
            <form action="{{ route('vehicle-bookings.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="start_time" value="{{ old('start_time') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">วัน-เวลา สิ้นสุด <span class="text-red-500">*</span></label>
                        <input type="datetime-local" name="end_time" value="{{ old('end_time') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg" required>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">สถานที่ไปราชการ / จุดหมาย <span class="text-red-500">*</span></label>
                        <input type="text" name="destination" value="{{ old('destination') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg" placeholder="ระบุสถานที่..." required>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">วัตถุประสงค์ <span class="text-red-500">*</span></label>
                        <textarea name="purpose" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg" placeholder="ระบุภารกิจ..." required>{{ old('purpose') }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="vehicle_driver_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg">
                            ...
                            @foreach ($drivers as $driver)
                                <option value="{{ $driver->id }}" {{ old('vehicle_driver_id') == $driver->id ? 'selected' : '' }}>
