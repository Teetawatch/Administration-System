<x-app-layout>
    <x-slot name="header"><h1 class="text-xl font-semibold text-gray-800">เพิ่มข่าวราชนาวี</h1></x-slot>
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <a href="{{ route('navy-news.index') }}" class="hover:text-navy-600">ข่าวราชนาวี</a>
        <i data-lucide="chevron-right" class="w-4 h-4"></i><span class="text-gray-800">เพิ่มใหม่</span>
    </div>
    <div class="card max-w-4xl">
        <div class="card-header"><h3 class="text-lg font-semibold text-gray-800">กรอกข้อมูลข่าว</h3></div>
        <div class="card-body">
            <form action="{{ route('navy-news.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">เลขที่ข่าว <span class="text-red-500">*</span></label>
                        <input type="text" 
                               name="news_number" 
                               value="{{ old('news_number', $nextNewsNumber) }}" 
                    ...
                    <div>
                        ...
                        <input type="date" name="news_date" value="{{ old('news_date', date('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500" required>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">หัวข้อ <span class="text-red-500">*</span></label>
                        <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500" required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="file" name="attachment" accept=".pdf,.doc,.docx,.jpg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-navy-50 file:text-navy-700">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">เนื้อหา</label>
                        <textarea name="content" rows="4" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500">{{ old('content') }}</textarea>
                    </div>
                </div>
